	...
	public static function generateHTMLPages($formId, $formName, $formType = "", $topContent = "", $bottomContent = "", $showTitle = false, $showDes = false)
	{
		...
		if ($items)
		{
				...
				if ($i + 1 == count($formPages))
				{
					...
					if (!empty($items->form_captcha) && $items->form_captcha == 1)
					{
						...
						if(version_compare(@$params->version, '2.0', '>=')){
							...
							//$dispatcher = JEventDispatcher::getInstance();
							//$dispatcher->trigger('onInit', $captchaId);
							...
							$html .= '<script type="text/javascript">try {var onloadCallback = function() {'
								. 'grecaptcha.render("' . $captchaId . '", {sitekey: "' . $dataSiteKey . '", theme: "' . $dataTheme . '"});'
								. '}}catch(err){}</script>';
							$html .= "<div id=\"" . $captchaId . "\"  data-sitekey=\"" . $dataSiteKey . "\" data-theme=\"". $dataTheme ."\" class=\"control-group g-recaptcha\"></div>";
						}else{
			...
			foreach ($getHeadData['scripts'] as $script => $option)
			{
				...
				{
					if (!empty($checkLoadJS['google-maps']) && empty($getHeadData['scripts']['https://maps.google.com/maps/api/js?v=3.23&libraries=places']))
					{
						if ($uri->getScheme() == 'https')
						{
							$scripts['https://maps.google.com/maps/api/js?v=3.23&libraries=places'] = $option;
						}
						else
						{
							$scripts['http://maps.google.com/maps/api/js?v=3.23&libraries=places'] = $option;
						}
			...
			if ($checkLoadScript)
			{
				...
				if (!empty($checkLoadJS['google-maps']))
				{
					if ($uri->getScheme() == 'https')
					{
						$document->addScript('https://maps.google.com/maps/api/js?v=3.23&libraries=places');
					}
					else
					{
						$document->addScript('http://maps.google.com/maps/api/js?v=3.23&libraries=places');
					}
